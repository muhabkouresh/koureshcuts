...
export async function POST(request: NextRequest) {
  ...
  // Only still-relevant entries count toward the position (flexible ones and
  // those whose requested day hasn't passed yet).
  const startOfToday = zonedToUtc(
    todayInTz(siteConfig.timezone),
    0,
    siteConfig.timezone,
  );
  const relevant = {
    OR: [{ date: null }, { date: { gte: startOfToday } }],
  };
